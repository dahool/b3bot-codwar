...
class PoweradminurtPlugin(b3.plugin.Plugin):
  ...
  def LoadMatchMode(self):
    ...
    self.matchmode_cvar = {}
    try:
      for e in self.config.get('pamatch_settings/cvar'):
        if not self.matchmode_cvar.has_key(e.attrib['status']):
          self.matchmode_cvar[e.attrib['status']] = []
        self.matchmode_cvar[e.attrib['status']].append({e.attrib['name']: e.text})
    except:
      self.warning('Can\'t read matchmode config')
...
# shuffleteams
  ...
  def cmd_pamatch(self, data, client, cmd=None): 
    ...
    if not data or data not in ('on','off'):
      ...
      try:
        for e in self.matchmode_cvar[data]:
          for k,v in e.items():
            self.debug('%s: %s' % (k,v))
            self.console.write('%s %s' % (k,v))
      except Exception, e:
        self.error(e)
        client.message(str(e))
...
#------------- SGT --------------------------------------------
  ...
  def set_configmode(self, mode):
    sett = {}
    try:
        self.debug('reading default settings')
        for e in self.config.get('gametype_default/cvar'):
            self.verbose('%s: %s' % (e.attrib['name'],e.text))
            sett[e.attrib['name']] = e.text
    except:
        self.debug('Unable to read default settings')

    try:
        self.debug('reading settings for %s' % mode)
        for e in self.config.get('gametype_%s/cvar' % mode):
            self.verbose('%s: %s' % (e.attrib['name'],e.text))
            sett[e.attrib['name']] = e.text
    except:
        self.debug('Unable to read settings for %s' % mode)
            
    try:
        self.debug('Applying %d settings' % len(sett))
        for key, value in sett.iteritems():
            self.debug('%s %s' % (key,value))
            self.console.write('%s %s' % (key,value))
    except Exception,e:
        self.error(e)
