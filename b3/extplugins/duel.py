...
class DuelPlugin(b3.plugin.Plugin):
    ...
    def onStartup(self):
        ...
        self._adminPlugin.registerCommand(self, 'duelstat', 1, self.cmd_duelstat)
    ...
    def cmd_duel(self, data, client, cmd=None):
        ...
        if client == opponent:
            client.message('you cannot duel yourself n00b')
            return
        if client.team == opponent.team:
            client.message('you cannot duel a teammate')
            return
        if client.team == b3.TEAM_SPEC or opponent.team == b3.TEAM_SPEC:
            client.message('dueling with a spect, yeah sure.')
            return
    ...
    def cmd_duelstat(self, data, client, cmd=None):
        """\
        [<name>] - show current score
        """
        duels = client.var(self, 'duelling', {}).value
        if len(duels) == 0:
            client.message('you started no duel. Nothing to show')
            return
        # this will split the player name and the message
        input = self._adminPlugin.parseUserCmd(data)
        if not input:
            ...
            if not opponent:
                ...
                return
            if opponent not in duels:
                client.message('^7You have no duel with %s' % opponent.exactName)
            else:
                duels[opponent].announceScoreTo(client)
                duels[opponent].announceScoreTo(opponent)
